...
| **A3** | ✅ Complete | Replace `FormatHint::detect` `todo!` with heuristic scoring (JSON line ratio, `=`, syslog PRI, CLF patterns) | §5.1 | **M** | Lock format after sample; per-line fallback to raw as spec states. |
...
| **B5** | ✅ Complete | Multiline: implement `fold_multiline` and integrate `MultilineConfig` in line iteration (indent vs `--multiline-start`) | §18.1 | **M** | Stack traces / continuation lines become one record; explicit `--multiline-start` takes precedence over indentation heuristic. |
| **B6** | ✅ Complete | Track `RunStats.unparseable_lines` and `verbose` stats for fallthrough lines | §5.1, §15 | **S** | Increment counters consistently across parser fallbacks; verbose text output surfaces totals without changing quiet mode. |
